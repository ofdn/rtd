...
<p class="description">Where available, a record's Identifiers section lists a Wikidata QID, VIAF and ISNI (name authority files used by libraries worldwide), LC/NAF (Library of Congress), GND (the German national authority file), and a WorldCat Entity id. These are cross-references, a distinct concept from <code>sources</code> (see the sourcing policy in CONTRIBUTING.md). They let a researcher or another database confirm that a person or typeface here is the same one referenced elsewhere, and let systems like Wikidata eventually cite RTD as an independent authority in its own right.</p>
